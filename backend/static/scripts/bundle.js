(() => {
  var t = {
      432: (t) => {
        t.exports = {
          ...
          REDIRECT_TO_GAME_ROOM: "redirect-to-game-room",
          PLAY_CARD: "play-card",
          DRAW_CARD: "draw-card",
          CALL_UNO: "call-uno",
        };
  ...
  (s.d = (t, e) => {
    ...
    (() => {
      ...
      for (let t = 0; t < h.length; t++) p[h.charCodeAt(t)] = t;
      ...
      const v = g.setTimeout,
        _ = g.clearTimeout;
      function E(t, e) {
        e.useNativeTimers
          ? ((t.setTimeoutFn = v.bind(g)), (t.clearTimeoutFn = _.bind(g)))
        ...
        constructor(t, e = {}) {
          super(),
            ...
            t
              ...
              : e.host && (e.hostname = K(e.host).host),
            E(this, e),
            (this.secure =
              null != e.secure
                ? e.secure
      ...
      function Z(t) {
        return (
          (z && (t instanceof ArrayBuffer || G(t))) ||
          (Q && t instanceof Blob) ||
          (X && t instanceof File)
        );
      }
      ...
      function yt(t, e) {
        ...
        return (
          e.forceNew || e["force new connection"] || !1 === e.multiplex || o
            ? (a = new ft(n, e))
            : (mt[i] || (mt[i] = new ft(n, e)), (a = mt[i])),
          s.query && !e.query && (e.query = s.queryKey),
          a.socket(s.path, e)
        );
      }
      Object.assign(yt, { Manager: ft, Socket: dt, io: yt, connect: yt });
      const gt = (t) => {
        document.getElementById("msg-id") &&
          ((document.getElementById("msg-id").innerText = t),
          setTimeout(() => {
      ...
      const vt = yt(),
        ...
        Et = "../images/",
        kt = JSON.parse(localStorage.getItem("user")),
        wt = document.getElementById("quit-btn-id");
      wt &&
        wt.addEventListener("click", () => {
          vt.emit(bt.LEAVE_GAME, { game_id: _t, user: kt });
        }),
        vt.emit(bt.JOIN_GAME, { game_id: _t, user: kt }),
        vt.on(bt.REDIRECT_TO_GAME_ROOM, ({ game_id: t }) => {
          window.location.href = `/games/${t}`;
        }),
        vt.on(
          bt.CREATE_GAME,
          ({ gametitle: t, count: e, game_id: s, ongoing: n }) => {
            let i = document.getElementById("games-list-id");
            if (!i) return;
            let r = document.createElement("div");
            (r.style.backgroundColor = "rgb(139, 246, 210)"),
              (r.style.marginBottom = "1px"),
              (r.innerHTML = `<a class="game-room-link" href="/waitingroom/${s}">Title: ${t}, # ${s} Players: ${e}, Started: ${n}</a>`),
              i.appendChild(r);
          }
        ),
        vt.on(bt.CHAT, ({ message: t, username: e }) => {
          let s = document.getElementById("chat-list-id");
          if (!s) return;
          ...
          (i.style.backgroundColor = "rgb(59, 245, 149)"),
            (i.style.marginBottom = "1px");
          let r = document.createElement("span");
          (r.style.fontWeight = "bold"), (r.textContent = e);
          let o = document.createElement("span");
          (o.style.marginLeft = "5px"), (o.textContent = n);
        ...
        (e.innerHTML = ""),
          t.forEach((t) => {
            if (kt.id !== t.user_id) {
              ...
              (s.innerHTML = ""),
                (s.id = "opponent-hand-id"),
                (s.className = "opponent-hand");
              const n = document.createElement("p");
              (n.innerText = `Opponent: ${t.name}`),
                (n.className = "player-name"),
                t.hand.forEach((t) => {
                  const e = document.createElement("img");
                  e.setAttribute("src", `${Et}back.png`),
                    (e.id = t + "-id"),
                    e.setAttribute("class", "opponent-card"),
                    e.setAttribute("width", "100px"),
                    e.setAttribute("height", "135px"),
                    s.appendChild(e);
                }),
                e.appendChild(n),
                e.appendChild(s);
      ...
      vt.on(
        bt.START_GAME,
        ({ top_deck: t, top_discard: e, players: s, currentPlayerName: n }) => {
          console.log(n),
            document.getElementById("deck-img-id") &&
              document.getElementById("discard-img-id") &&
              document.getElementById("current-player-name-id") &&
              ((document.getElementById("deck-img-id").src = Et + t),
              (document.getElementById("discard-img-id").src = Et + e),
              (document.getElementById("current-player-name-id").innerText = n),
              At(s));
        }
      ),
        vt.on(bt.JOIN_GAME, ({ message: t, numPlayers: e }) => {
        ...
        vt.on(bt.LEAVE_GAME, ({ message: t, numPlayers: e }) => {
          document.getElementById("num-of-players-id") &&
            (_t &&
              0 !== _t &&
              (document.getElementById("num-of-players-id").innerText =
        ...
        vt.on(
          ...
          ({
            ...
            i && i === kt.id
              ...
              : ((document.getElementById("discard-img-id").src = Et + t),
        ...
        vt.on(
          bt.DRAW_CARD,
          ({ top_deck: t, players: e, currentPlayerName: s }) => {
            (document.getElementById("deck-img-id").src = Et + t),
              (document.getElementById("current-player-name-id").innerText = s),
              At(e);
          }
        ),
        vt.on(bt.CALL_UNO, ({ message: t }) => {
          (document.getElementById("call-uno-msg-id").innerText = t),
            setTimeout(() => {
