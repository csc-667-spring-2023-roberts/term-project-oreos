(() => {
  ...
  (s.d = (t, e) => {
    ...
    (() => {
        ...
        U = {
          websocket: class extends E {
            ...
            doOpen() {
              ...
              try {
                this.ws = V ? new H(t, e, s) : e ? new H(t, e) : new H(t);
              } catch (t) {
      ...
      function Z(t) {
        return (
          (J && (t instanceof ArrayBuffer || W(t))) ||
          (Q && t instanceof Blob) ||
          (X && t instanceof File)
        );
      }
      ...
      bt.emit(mt.JOIN_GAME, { game_id: vt, user: { name: "Jon", user_id: 1 } }),
        bt.on(mt.REDIRECT_TO_GAME_ROOM, ({ game_id: t }) => {
          window.location.href = `/games/${t}`;
        }),
        bt.on(
          mt.CREATE_GAME,
          ({ gametitle: t, count: e, user_id: s, game_id: n }) => {
            let i = document.getElementById("games-list-id");
            if (!i) return;
            let r = document.createElement("li");
            (r.innerHTML = `Name: <a href="/waitingroom/${n}">${t}</a>, Players: ${e}`),
              i.appendChild(r);
          }
        ),
        bt.on(mt.CHAT, ({ message: t, username: e }) => {
          let s = document.getElementById("chat-list-id");
          if (!s) return;
          let n = document.createElement("li");
          (n.innerHTML = `${e}: ${t}`), s.appendChild(n);
        }),
        bt.on(mt.START_GAME, ({ deck: t, discardPile: e }) => {
          const s = "../images/";
          document.getElementById("deck-img-id") &&
            document.getElementById("discard-img-id") &&
            ((document.getElementById("deck-img-id").src = s + t[0]),
            (document.getElementById("discard-img-id").src = s + e[0]));
        }),
        bt.on(mt.JOIN_GAME, ({ message: t, numPlayers: e }) => {
          console.log(t),
